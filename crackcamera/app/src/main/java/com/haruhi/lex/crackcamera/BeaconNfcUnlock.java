 ...
 * Does not rely on a manifest {@code TECH_DISCOVERED} filter, so tags scanned
 * outside unlock never launch {@link MainActivity}.
 */
final class BeaconNfcUnlock {
    ...
    /** Disable NFC listening while {@link MainActivity} is paused (required by the NFC APIs). */
    static void onHostPaused() {
        BeaconNfcUnlock unlock = sActive;
        if (unlock != null) {
            unlock.disableDispatch();
        }
    }

    /** Re-enable NFC listening when {@link MainActivity} resumes and unlock is still active. */
    static void onHostResumed() {
        BeaconNfcUnlock unlock = sActive;
        if (unlock != null && unlock.active && !unlock.completed) {
            unlock.enableDispatch();
        }
    }
    ...
    private void enableDispatch() {
        if (dispatchEnabled || !active || completed) {
            return;
        }
        NfcAdapter adapter = NfcAdapter.getDefaultAdapter(activity);
        if (adapter == null || !adapter.isEnabled()) {
